...
void BPTree::insert_key(const Key &key, const RecordPtr &record_ptr) {
    ...
    logger.log("potential split node ptr "+ potential_split_node_ptr);
...
void BPTree::delete_key(const Key &key) {
    TreeNode* root_node = TreeNode::tree_node_factory(this->root_ptr);
    logger.log("delete key function called in the root");
    root_node->delete_key(key);
    logger.log("in the root node");
    logger.log("root node size "+to_string(root_node->size));
    ...
    logger.log("before deleting");
    delete root_node;
    logger.log("after deleting");
}
